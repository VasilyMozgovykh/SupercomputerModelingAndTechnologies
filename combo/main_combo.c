...
double get_max_norm(double **vec, double *gammas, TaskSettings settings) {
    ...
    for (int y = 1; y < settings.local_dim.ydim - 1; y++) {
        for (int x = 1; x < settings.local_dim.xdim - 1; x++) {
            max_local = fmax(max_local, fabs(vec[y][x]));
        }
    }
    ...
    double max_global = 0.;
    for (int i = 0; i < settings.mpi.size; i++) {
        max_global = fmax(max_global, gammas[i]);
    }
    return max_global;
}
...
double get_l1_norm(double **vec, double *gammas, TaskSettings settings) {
    ...
    double l1_local = 0.;
    #pragma omp parallel for reduction(+: l1_local)
    for (int y = 1; y < settings.local_dim.ydim - 1; y++) {
        for (int x = 1; x < settings.local_dim.xdim - 1; x++) {
...
//     #pragma omp for private(xforward, xbackward, yforward, ybackward) nowait
//     for (int y = 1; y < settings.local_dim.ydim - 1; y++) {
//         for (int x = 1; x < settings.local_dim.xdim - 1; x++) {
...
TaskResults solve_poisson(TaskSettings settings) {
    ...
    double xstep = (XRIGHT - XLEFT) / settings.global_dim.xdim;
    double ystep = (YRIGHT - YLEFT) / settings.global_dim.ydim;
    int ydim_global = settings.global_dim.ydim;
    int xdim_global = settings.global_dim.xdim;
    int ydim_local = settings.local_dim.ydim;
    int xdim_local = settings.local_dim.xdim;
    ...
    #pragma omp parallel private(y, x)
    {
        // r_0
        #pragma omp for nowait
        for (y = 1; y < ydim_local - 1; y++) {
            for (x = 1; x < xdim_local - 1; x++) {
                r_vec[y][x] = settings.coef_F[y][x];
            }
        }
        ...
        for (y = 1; y < ydim_local - 1; y++) {
            for (x = 1; x < xdim_local - 1; x++) {
                z_vec[y][x] = settings.coef_D_inv[y][x] * r_vec[y][x];
            }
        }
    }
    ...
    #pragma omp parallel private(y, x)
    {
        ...
        for (y = 1; y < ydim_local - 1; y++) {
            for (x = 1; x < xdim_local - 1; x++) {
                xforward = settings.coef_a[y][x + 1] * (p_vec[y][x + 1] - p_vec[y][x]) / xstep;
                xbackward = settings.coef_a[y][x] * (p_vec[y][x] - p_vec[y][x - 1]) / xstep;

                yforward = settings.coef_b[y + 1][x] * (p_vec[y + 1][x] - p_vec[y][x]) / ystep;
                ybackward = settings.coef_b[y][x] * (p_vec[y][x] - p_vec[y - 1][x]) / ystep;

                q_vec[y][x] = (xbackward - xforward) / xstep + (ybackward - yforward) / ystep;
            }
        }
    }
    ...
    for (y = 1; y < ydim_local - 1; y++) {
        for (x = 1; x < xdim_local; x++) {
            w_vec[y][x] += alpha * p_vec[y][x];
        }
    }

    /* Calculate 0 step norms */
    ...
    for (iter_num = 1; iter_num <= max_iters && l2_norm >= TOL; iter_num++) {
        ...
        #pragma omp parallel private(y, x)
        {
            ...
            for (y = 1; y < ydim_local - 1; y++) {
                for (x = 1; x < xdim_local - 1; x++) {
                    p_vec[y][x] = z_vec[y][x] + beta * gamma * p_vec[y][x];
                }
            }
        }
        MPI_Barrier(MPI_COMM_WORLD); // Wait for all vectors

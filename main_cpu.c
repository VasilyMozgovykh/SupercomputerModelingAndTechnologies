...
void apply_laplace_operator(
    ...
    for (int idx = 0; idx < (xdim - 1) * (ydim - 1); idx++) {
        ...
        double center = vec[idx];
        double xforward = (w < (xdim - 2) ? vec[idx + 1] : 0.);
        double xbackward = (w > 0 ? vec[idx - 1] : 0.);
        double yforward = (h < (ydim - 2) ? vec[idx + (xdim - 1)] : 0.);
        double ybackward = (h > 0 ? vec[idx - (xdim - 1)] : 0.);
